...
ask_choice "Where should digests be delivered?" "telegram" "discord" "feishu" "log file only"
CHANNEL="$REPLY"

BOT_TOKEN="" CHAT_ID="" THREAD_ID="" DISCORD_WEBHOOK=""
FEISHU_APP_ID="" FEISHU_APP_SECRET="" FEISHU_CHAT_ID="" FEISHU_THREAD_ID=""
...
case "$CHANNEL" in
  ...
  feishu)
    echo ""
    dim "  Create a Feishu app at https://open.feishu.cn/app and add it to your group."
    ask "App ID (cli_xxxxxxxxxx)" ""
    FEISHU_APP_ID="$REPLY"
    ask "App Secret" ""
    FEISHU_APP_SECRET="$REPLY"
    echo ""
    dim "  Your Feishu group/chat ID (oc_xxxxxxxxxx)."
    ask "Chat ID" ""
...
if [[ ! -f "$CONFIG_FILE" ]]; then
  ...
  [[ "$CHANNEL" == "feishu"   ]] && FEISHU_ENABLED="true"   && LOG_ENABLED="false"
